...
set __=.
cmdgfx_input.exe m0nW8xR | call %0 %* | cmdgfx_gdi "" Sf2:0,0,360,75,120,75
set __=
...
set /a W=120, WWW=W*3, H=75, W150=W+30, W200=W+80
...
set HELP=text b 0 0 _ENTER=auto/mouse,_SPACE=color,_c=clear,_Up/Down=size,_n=negative,_1-3=nof_bobs,_p=pause,_h=hide_help_ 8,73
...
for /L %%1 in (1,1,300) do if not defined STOP for %%c in (!COL!) do (
   ...
   if !MODE! == 0 (
		...
		if "!RESIZED!"=="1" set /a "W=SCRW, WWW=W*3, H=SCRH, XMID=W/2, YMID=H/2, HLPY=H-3, HLPX=W/2-102/2, W150=W+30, W200=W+80, XMUL=(W-20)/4, YMUL=(H-20)/3, XMUL2=(W-30)/4, YMUL2=(H-20)/4" & cmdwiz showcursor 0 & set HELP=text b 0 0 _ENTER=auto/mouse,_SPACE=color,_c=clear,_Up/Down=size,_n=negative,_1-3=nof_bobs,_p=pause,_h=hide_help_ !HLPX!,!HLPY!
