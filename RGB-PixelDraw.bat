...
for /L %%1 in (1,1,300) do if not defined STOP (
	...
	if not "!EV_BASE:~0,1!" == "N" (
		if !M_EVENT!==1 (
			...
			if !M_LB! == 1 (if !MOUSE_D!==0 set /a MOUSE_D=1,TSTX=BORD+WB, TSTY=DG+GUI_H & if !M_X! gtr !BORD! if !M_Y! gtr !DG! if !M_Y! lss !TSTY! if !M_X! lss !TSTX! if !FGUI!== 1 set /a INGUI=1) & (if !INGUI!==0 (if !COLPICK!==0 set DRAW="ipoly !CURRCOL0! 0 db 20 !OUTP:~0,-1!") & (if !COLPICK!==1 call :COLPICK !M_X! !M_Y!) ) & if !INGUI!==1 call :PROCESS_GUI
