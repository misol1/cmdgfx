...
for /L %%1 in (1,1,300) do if not defined STOP (
	echo "cmdgfx: fbox 2 0 20 0,0,%W%,%H% & image capture-1.gxy 2 0 0 -1 0,0 & 3d %FL% 3,-1 !RX!,!RY!,!RZ! -60,1520,0 1,2,1,0,0,0 0,0,0,0 %XMID%,!YMID!,20000,%ASPECT% a 0 b1 2 0 b1" W12f0:0,0,%W%,50
...
echo F>serverflags.dat
echo "cmdgfx: f0:0,0,%W%,50" f0:0,0,%W%,%H%
...
for /L %%1 in (1,1,300) do if not defined STOP for %%p in (!P_I!) do (
	...
	echo "cmdgfx: image capture-1.gxy 2 0 0 -1 0,0 & 3d %FN% 3,-1 !RX!,!RY!,!RZ! !X!,!Y!,0 1,1,1,0,0,!ZD! 0,800,0,0 %XMID%,!YMID!,!DIST!,%ASPECT% !COL! 0 fe & text e 0 0 Score:_!SCORE!_\e0(!HISCORE!) 2,1 & !ERR! & skip %EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%" DW16-n

	set /A ZD+=!ADDVAL!
	rem Give server chance to actually read some extra keys
	echo "cmdgfx: " W0n
	echo "cmdgfx: " W0n
	
	if exist EL.dat (
		set /a KCNT=0
		for /F "tokens=* delims= " %%f in (EL.dat) do (
			set /a KEY=%%f		
			...
			rem set /a KCNT+=1 & if !KCNT! == 2 echo More than one key & cmdwiz delay 500
		)
