...
@echo off
bg font 0 & cls & cmdwiz showcursor 0 & mode 240,100
if defined __ goto :START
set __=.
call %0 %* | cmdgfx_gdi.exe "" Sf0:0,0,240,100W20
set __=
...
for /L %%1 in (1,1,300) do if not defined STOP (
	...
	echo "cmdgfx: fbox 0 0 20 0,0,%W%,%H% & %BG:~1,-1% & 3d starfield0.ply 1,1 0,0,0 !TX!,0,0 10,10,10,0,0,0 0,0,2000,10 %XMID%,%YMID%,%SDIST%,%ASPECT% !COLS! & 3d starfield1.ply 1,1 0,0,0 !TX2!,0,0 10,10,10,0,0,0 0,0,2000,10 %XMID%,%YMID%,%SDIST%,%ASPECT% !COLS! & !PLY:~1,-1! & !NMY:~1,-1! & !SHOTS:~1,-1! & text 7 1 0 SCORE:_!SCORE!_(!HISCORE!) 2,1 & skip %EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%"

	set /A PLXB=!PLXPOS!+18, PLXB2=!PLXPOS!-19
	set /A PLYB=!PLYPOS!+16, PLYB2=!PLYPOS!-15
	for /L %%a in (1,1,!NOF_ENEMIES!) do if !EX%%a! lss !PLXB! if !EX%%a! gtr !PLXB2! if !ENEMY%%aY! lss !PLYB! if !ENEMY%%aY! gtr !PLYB2! cmdwiz flushkeys & call :GAMEOVER & set /a STOP=1&if !GAMECHOICE! == 0 set /a RESTARTGAME=1
	...
	if exist EL!ELCNT!.dat (for /f "tokens=1,2,3,4,5,6" %%A in (EL!ELCNT!.dat) do ( set /a ACTIVE_KEY331=%%A,ACTIVE_KEY328=%%B,ACTIVE_KEY333=%%C,ACTIVE_KEY336=%%D,ACTIVE_KEY27=%%E,ACTIVE_KEY32=%%F >nul 2>nul )) >nul 2>nul
	
	set /a ELCNT+=1 & if !ELCNT! geq 3 set /a ELCNT=0
	start "" /B /HIGH cmdwiz getkeystate 25h 26h 27h 28h 27 32>EL!ELCNT!.dat
	...
	if !ACTIVE_KEY27!==1 set STOP=1
	if !ACTIVE_KEY32!==1 if !SPACEDOWN! == 0 set /a SHLEFT=1 & for /L %%a in (1,1,%NOF_SHOTS%) do if !SHLEFT!==1 if !SHOT%%aA!==0 set /a SHLEFT=0,SHOT%%aA=1,SHOT%%aX=!PLXPOS!+14,SHOT%%aY=!PLYPOS!+10, SPACEDOWN=1
	if !ACTIVE_KEY32!==0 set /a SPACEDOWN=0 
