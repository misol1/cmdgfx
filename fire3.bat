...
cmdgfx_gdi "fbox f 0 30 0,0,%WW%,130 & fbox f 0 20 0,0,%WW%,120 & fbox e 0 20 %W%,0,%W%,95" Pf2:0,0,%WW%,130
set OUT=""&(for /L %%a in (1,1,75) do set /a "X=!RANDOM! %% %W%+%W%,FG=!RANDOM! %% 6+10" & set OUT="!OUT:~1,-1! & pixel !FG! 0 30 !X!,126")&cmdgfx_gdi !OUT! Pf2:0,0,%WW%,130
...
for /L %%1 in (1,1,300) do if not defined STOP for %%c in (!COL!) do (
	...
	start /B /high cmdgfx_gdi "!OUT:~1,-1! & block 0 %W%,1,%W%,125 %W%,0 -1 0 0 !STREAM:~1,-1! & block 0 %W%,0,%W%,130 0,-2 -1 0 0 !TRANSF%%c:~1,-1!" Pf2:0,0,%WW%,130
	cmdgfx "" W14kn
	set KEY=!errorlevel!
	
	if !KEY! == 102 cmdgfx_gdi "image img\fire.txt f 0 0 20 87,79 & image img\fire.txt f 0 0 20 89,79 & image img\fire.txt f 0 0 20 88,78 & image img\fire.txt f 0 0 20 88,80 & image img\fire.txt f 0 0 20 88,79" Pf2:0,0,%WW%,130
