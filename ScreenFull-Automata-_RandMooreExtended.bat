...
set __=.
set /a density=50
call %0 %* | cmdgfx_gdi "fbox 0 0 db" %TOP%m0OW0Sfa:0,0,%W%,%H%nt8
set __=
...
for /l %%a in () do (
	...
	if !rateCnt! geq !updateRate! (
		...
		echo "cmdgfx: block 0 0,0,%w%,%h% 0,0 -1 0 0 - DLL:eextern:randMooreExtended:!stay!,!born!,!slowDeath!,!liveCol!,!NH! & !XTRP:~1,-1! & !TS! text b 0 0 !stay!__!born!__!density!__(!slowDeath!_!liveCol!)_!NH!___(!ifi!/!iflc!) !XTXT!,!YTXT! 5 & !FS! text b 0 0 [FRAMECOUNT] 2,28 5 & skip %EXTRA%%EXTRA%%EXTRA%%EXTRA%" f!font!:0,0,%W%,%H%,!WSC!,!HSC!,!XSC!,!YSC!
		set /a rateCnt=0
	) else (
	...
	if !KEY! neq 0 (
		...
		if !LKEY! == "n" set /a NH-=1 & (if !NH! lss 0 set /a NH=13) & call :PREP 1
		if !LKEY! == "N" set /a NH+=1 & (if !NH! gtr 13 set /a NH=0) & call :PREP 1
...
if !savePal! == 0 cmdwiz print "::set /a stay=!stay!,born=!born!,density=!density!,liveCol=!liveCol!,slowDeath=!slowDeath!,NH=!NH!,opType=!opType!%WVS% !OPOUT:~1,-1!\n" >> data\randMoore-Extended-Set.txt
if !savePal! == 1 cmdwiz print "::set /a stay=!stay!,born=!born!,density=!density!,liveCol=!liveCol!,slowDeath=!slowDeath!,NH=!NH!,opType=!opType!%WVS% !OPOUT:~1,-1!& set PAL=!PAL!\n" >> data\randMoore-Extended-Set.txt
set WVS=
goto :eof

:GETPATTERNFROMFILE
set inFile=data\randMooreExtended-data.txt
...
set /a cnt=0, initPrep=1, WVAL=0, updateRate=0, opType=0
set PAL=
