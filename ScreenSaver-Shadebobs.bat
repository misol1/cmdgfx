...
for /L %%1 in (1,1,300) do if not defined STOP for %%c in (!COL!) do (
	...
	echo "cmdgfx: !DRAW:~1,-1! & block 0 !W200!,0,!W150!,!H! !W200!,0 -1 0 0 ?1??=?0?? & block 0 !W200!,0,!W200!,!H! 0,0 -1 0 0 !PAL%%c!" Ff2:0,0,!WWW!,!H!,!W!,!H!
	...
	if !a! geq !TIMEOUT! (
		set /a "SIZE=1, RS=!RANDOM! %% 4, BS=!RANDOM! %% 9, NOFSB=2 + !RANDOM! %% 2, TIMEOUT=3500+!RANDOM! %% 1500"
		if !DRWMODE!==9 set /a RS=0
		if !RS! lss 3 echo "cmdgfx: fbox 0 0 db"& set /a "OLDCOL=COL, COL=!RANDOM! %% 3, DRWMODE=8" & if !COL!==!OLDCOL! set /a "COL=!RANDOM! %% 3"
		if !RS! == 3 set /a DRWMODE=9
		if !BS! == 8 set /a SIZE=2, NOFSB-=1
		set t1=!time: =0!
	)
	...
	set DRAW="ipoly 1 0 ? !DRWMODE! !OUTP:~0,-1! & !SS2! ipoly 1 0 ? !DRWMODE! !OUTP2:~0,-1!& !SS3! ipoly 1 0 ? !DRWMODE! !OUTP3:~0,-1!"
