...
for /L %%1 in (1,1,300) do if not defined STOP (
	set /a COLCNT+=2,COLCNT2+=5 
	
	cmdgfx!RENDERER! "block 0 0,0,70,50 110,0 -1 0 0 %STREAM% sin((x+!COLCNT!/1)/110)*13*sin((y+!COLCNT2!/5)/65)*8 & 3d objects\!FNAME! !DRAWMODE!,-1 !RX!,!RY!,!RZ! 0,0,0 !MOD!,0,0,10 %XMID%,%YMID%,!DIST!,%ASPECT% !PAL!" kZ300f1:0,0,181,%H%
	
	set KEY=!ERRORLEVEL!
	if !ROTMODE! == 0 set /a RX+=4&set /a RY+=7&set /a RZ-=6
