...
set /a DIV=2 & set /a XMID=%W%/2/!DIV!,YMID=%H%/2/!DIV!, XMUL=%W%/2/!DIV!, YMUL=%H%/2/!DIV!, SXMID=%W%/2,SYMID=%H%/2, DELAY=12
...
rem set /a P1=0,P2=3,P3=0,P4=0,P5=-2,P6=-3,P7=-3,P8=4,SC=35962,CC=7098,SC2=-26481,CC2=-5009,SC3=61416,CC3=-41598,SC4=-32963,CC4=26096
rem set /a P1=-2,P2=3,P3=2,P4=1,P5=2,P6=1,P7=4,P8=-2,SC=24454,CC=15765,SC2=-25419,CC2=4724,SC3=56698,CC3=-30342,SC4=-29833,CC4=23158
set /a P1=-1,P2=2,P3=-3,P4=0,P5=2,P6=-3,P7=-1,P8=3,SC=25047,CC=17312,SC2=-20657,CC2=5003,SC3=62725,CC3=-30318,SC4=-30942,CC4=24972
...
for /L %%1 in (1,1,300) do if not defined STOP (
	...
	if !a! geq 1 (
	   ...
	   for %%a in (!DRAWOP!) do set DRAW=!D%%a!
		...
		if !STARTCNT! lss 0 if !DIV! == 1 start /B /High cmdgfx_gdi "fbox !COL! 0 00 0,0,%W%,%H% & !STR:~1,-1!" kOf0:0,0,!W!,!H! !PALETTE!
		if !STARTCNT! lss 0 if !DIV! == 2 start /B /High cmdgfx_gdi "fbox !COL! 0 00 0,0,%W%,%H% & !STR:~1,-1! & block 0 0,0,%SXMID%,%SYMID% %SXMID%,0 -1 1 0 & block 0 0,0,%SXMID%,%SYMID% 0,%SYMID% -1 0 1 & block 0 0,0,%SXMID%,%SYMID% %SXMID%,%SYMID% -1 1 1" kOf0:0,0,!W!,!H! !PALETTE!

		if exist EL.dat set /p KEY=<EL.dat 2>nul & del /Q EL.dat >nul 2>nul & if "!KEY!" == "" set KEY=0
		
		if !KEY! == 32 set /a CHANGECOUNT=!CHANGESTEPS! & (for /L %%a in (1,1,8) do set /a "P%%a=!RANDOM! %% 9 - 4") & for /L %%a in (1,1,%NOFLINES%) do set LN%%a= 
