...
set /a W=120, H=70
set /a W2=80, H2=52, XMID3=W/2+2, HH=35, YMID3=HH/2-1, DIST3=4000
set /a RX=700, RY=0, RZ=0, TXTX=9, TXTXP=TXTX+1, TXTY=9, TXTYP=TXTY+1
set TEXT=Alien_Intruders\g21
set C=3366ee,
...
for /L %%1 in (1,1,300) do if not defined STOP (
  ...
  echo "cmdgfx: fbox 2 0 fa & 3d objects\spaceship.obj 3,0 !RY!,30,!RZ! 0,0,0 100,100,100,0,1,-1 0,0,0,0 !XMID3!,!YMID3!,%DIST3%,0.66 0 0 0 & text 0 0 = %TEXT% !TXTX!,!TXTY! 1 & text 3 0 fe %TEXT% !TXTXP!,!TXTYP! 1" f6:0,!HH!,!WPP!,!HHPP! 000000,%C%808080,ffffff,%C%%C%%C%%C%%C%%C%%C%%C%%C%%C%%C%%C%

  set /p INPUT=
  for /f "tokens=1,2,4,6, 24,26,28" %%A in ("!INPUT!") do ( set EV_BASE=%%A & set /a K_EVENT=%%B, K_DOWN=%%C, KEY=%%D, RESIZED=%%E, SCRW=%%F, SCRH=%%G 2>nul )
  
  if "!RESIZED!"=="1" set /a "W=SCRW, H=SCRH, W2=((W*500)/1000+1)*8, W2PP=W2+1, H2=((H*500)/1000)*12,HH=H/2, HHPP=HH+1, WPP=W+1, XMID3=W/2+2, YMID3=HH/2-1, TXTX=W/2-16*6/2, TXTXP=TXTX+1, TXTY=HH/2-7, TXTYP=TXTY+1" & cmdwiz showcursor 0
