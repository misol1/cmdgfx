...
// 6. Code fix: Goraud shade rounding/edge bug
// 7. Code fix: ipoly when coords outside of screen sometimes causes holes in polygon (e.g. when zooming in glenz.bat)
...
int readCmdGfxTexture(Bitmap *bmap, char *fname) {
	...
	if (strstr(fname, ".pcx"))
		...
		res = readGxy(fname, bmap, (Bitmap *)bmap->extras, &w, &h, 0, -1, 1);
	}
	return res;
}
  ...
  if (argc > 2) {
	   char *fnd, fin[64];
		...
		for (i=0; i < strlen(argv[2]); i++) {
			switch(argv[2][i]) {
				case 'f': 
				i++; fontIndex = argv[2][i] - '0';
				if (fontIndex < 0 || fontIndex > 9) fontIndex = 6; 
