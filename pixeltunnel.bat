...
for /L %%1 in (1,1,600) do if not defined STOP (
	set CRSTR=""
	for /L %%a in (1,1,%NOF%) do (for %%b in (!PR%%a!) do for %%c in (!PRC%%a!) do set /a INV=%%b,INV2=%%c & set /A "XP=(%SINE(x):x=!INV!/2*31416/180%*((PZ%%a+%BDIST%)/!DIVROTX!)>>%SHR%),YP=(%SINE(x):x=!INV2!/2*31416/180%*((PZ%%a+%BDIST%)/!DIVROTY!)>>%SHR%),PZ%%a-=!SPEED!,PR%%a+=%ADD1%,PRC%%a+=%ADD2%,C=(PZ%%a+%BDIST%)/%COLDIV%") & (for %%d in (!C!) do set CRSTR="!CRSTR:~1,-1! & 3d %WNAME% %DRAWMODE%,0 0,0,!CRZ! 0,0,0 1,1,1,!XP!,!YP!,!PZ%%a! 0,0,0,10 %XMID%,%YMID%,%DIST%,%ASPECT% !C%%d!")&(if !PZ%%a! leq -%BDIST% set PZ%%a=%DISTMAX%&set /A PR%%a+=%NOF%*%ADD1%,PRC%%a+=%NOF%*%ADD2%)
