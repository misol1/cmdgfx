...
mode %F6W%,%F6H%
set __=.
call %0 %* | cmdgfx_gdi "" Sfa:0,0,720,480Z600
set __=
mode 80, 50
...
for /l %%a in (0,1,%NOF_O%) do for /l %%b in (0,1,5) do if not !t%%a!==M if not !t%%a!==N set /a f0=!F%%b_0!+%%a*8+1&set /a f1=!F%%b_1!+%%a*8+1&set /a f2=!F%%b_2!+%%a*8+1&set /a f3=!F%%b_3!+%%a*8+1,MODDER=%%a %% 4&echo f !f0!/1/ !f1!/2/ !f2!/3/ !f3!/4/ >>%FN%&(if !MODDER!==0 echo usemtl img\dos_shade.pcx >>%FN%)&(if !MODDER!==1 echo usemtl img\dos_shade2.pcx >>%FN%)&(if !MODDER!==2 echo usemtl img\dos_shade3.pcx >>%FN%)&(if !MODDER!==3 echo usemtl img\dos_shade4.pcx >>%FN%)
