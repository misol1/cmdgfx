...
cmdwiz fullscreen 0
cmdwiz setfont 6 & mode %F6W%,%F6H% & cls & title 3d test auto scale
cmdwiz showcursor 0
if defined __ goto :START
set __=.
cmdgfx_input.exe m0nuW10xR | call %0 %* | cmdgfx_gdi "" eSf0:0,0,%W%,%H%N250
set __=
...
for /L %%1 in (1,1,400) do if not defined STOP for %%o in (!DRAWMODE!) do (
	echo "cmdgfx: fbox 8 0 . 0,0,!W!,!H! & !MSG:~1,-1! & 3d objects\!FNAME! !DRAWMODE!,!O%%o! !RX!,!RY!,!RZ! 0,0,0 !MOD!,-4000,0,10 !XMID!,!YMID!,!DIST!,%ASPECT% !PAL!" Ff0:0,0,!W!,!H!
...
set ERR=
set PAL0=f 0 db  f b b1  b 0 db  b 7 b1  7 0 db  9 7 b1  9 0 db  9 1 b1  1 0 db  1 0 b1
if %OBJINDEX% == 0 set FNAME=tetrahedron.ply& set MOD=-1,-1,-1, 0,0,0 1
if %OBJINDEX% == 1 set FNAME=cube.ply& set MOD=-1,-1,-1, 0,0,0 1
if %OBJINDEX% == 2 set FNAME=icosahedron.ply& set MOD=-1,-1,-1, 0,0,0 1
if %OBJINDEX% == 3 set FNAME=shark.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 4 set FNAME=elephav.obj& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 5 set FNAME=airplane.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 6 set FNAME=ant.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 7 set FNAME=dolphins.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 8 set FNAME=scissors.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 9 set FNAME=steeringweel.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 10 set FNAME=teapot.ply& set MOD=1,1,1, 0,0,0 0
if %OBJINDEX% == 11 set FNAME=trashcan.ply& set MOD=1,1,1, 0,0,0 0
if %OBJINDEX% == 12 set FNAME=urn2.ply& set MOD=1,1,1, 0,0,0 1
if %OBJINDEX% == 13 set FNAME=torus.plg& set MOD=-1,-1,-1, 0,0,0 0
if %OBJINDEX% == 14 set FNAME=sphere.plg& set MOD=-1,-1,-1, 0,0,0 0
if %OBJINDEX% == 15 set FNAME=springy1.plg& set MOD=-1,-1,-1, 0,0,0 0
if %OBJINDEX% == 16 set FNAME=chopper.plg& set MOD=-1,-1,-1, 0,0,0 1
