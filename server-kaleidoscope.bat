...
set HELPMSG=text 7 0 0 SPACE\-ENTER\-m\-f\-p\-h 1,%HLPY%
...
for /L %%_ in (1,1,300) do if not defined STOP (
	...
	if "!RESIZED!"=="1" set /a "W=SCRW*2*rW/100+2, H=SCRH*2*rH/100+3, XMID=W/2, YMID=H/2, HLPY=H-4, DIST=4000-(W-222)*7, TRIDIST=7000-(W-222)*17, CUPOS=35+(W-222)/7+3" & set FN=%FN0%& (if !W! gtr 300 set FN=%FN1%)& (if !W! gtr 400 set FN=%FN2%) & cmdwiz showcursor 0 & set HELPMSG=text 7 0 0 SPACE\-ENTER\-m\-f\-p\-h 1,!HLPY! & if !SHOWHELP!==1 set MSG=!HELPMSG!
