...
set OBJ0=torus&set OBJ1=sphere&set OBJ2=double-sphere&set OBJ3=cube
...
for /L %%1 in (1,1,300) do if not defined STOP for %%c in (!COLCNT!) do for %%o in (!OBJCNT!) do (
	echo "cmdgfx: fbox 7 0 20 0,0,%W%,%H% & 3d objects\plot-!OBJ%%o!.ply %DRAWMODE%,1 !CRX!,!CRY!,!CRZ! 0,0,0 !SCALE!,0,0,0 0,0,0,10 %XMID%,%YMID%,!DIST!,%ASPECT% !COLS_%%c! & %HELP% & skip %EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%%EXTRA%"
	...
	if !KEY! == 32 set SCALE=%SCALE0%& set /A OBJCNT+=1& (if !OBJCNT!==3 set SCALE=%SCALE1%) & if !OBJCNT! gtr 3 set /a OBJCNT=0
